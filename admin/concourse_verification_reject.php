...
if (isset($_GET['id'])) {
    ...
    if (mysqli_num_rows($check_existing_result) > 0) {
        ...
        $update_query = "UPDATE concourse_verification SET status = 'rejected' WHERE concourse_id = '$concourse_id'";

        if (mysqli_query($con, $update_query)) {
            // Retrieve the concourse details
            $concourseDetailsQuery = "SELECT * FROM concourse_verification WHERE concourse_id = '$concourse_id'";
            $concourseDetailsResult = mysqli_query($con, $concourseDetailsQuery);

            if ($concourseDetailsResult && $concourseDetailsRow = mysqli_fetch_assoc($concourseDetailsResult)) {
                // Store the rejected concourse details in a session variable
                $_SESSION['rejected_concourse'] = $concourseDetailsRow;
            }
            ...
            if ($userVerificationResult && $userVerificationRow = mysqli_fetch_assoc($userVerificationResult)) {
                ...
                $notificationMessage = 'Your Concourse application has been rejected.';
